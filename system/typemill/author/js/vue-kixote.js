...
const kixote = Vue.createApp({
	...
	data() {
		return {
			...
			icons: {
				...
				Info: 		"M16 4C8.268 4 2 10.268 2 18s6.268 14 14 14 14-6.268 14-14S23.732 4 16 4zm0 6a2 2 0 110 4 2 2 0 010-4zm2 16h-4v-2h1v-6h-1v-2h3v8h1v2z",
				Help: 		"M22 12c0 4.422-4.922 8-11 8-0.953 0-1.875-0.094-2.75-0.25-1.297 0.922-2.766 1.594-4.344 2-0.422 0.109-0.875 0.187-1.344 0.25h-0.047c-0.234 0-0.453-0.187-0.5-0.453v0c-0.063-0.297 0.141-0.484 0.313-0.688 0.609-0.688 1.297-1.297 1.828-2.594-2.531-1.469-4.156-3.734-4.156-6.266 0-4.422 4.922-8 11-8s11 3.578 11 8zM28 16c0 2.547-1.625 4.797-4.156 6.266 0.531 1.297 1.219 1.906 1.828 2.594 0.172 0.203 0.375 0.391 0.313 0.688v0c-0.063 0.281-0.297 0.484-0.547 0.453-0.469-0.063-0.922-0.141-1.344-0.25-1.578-0.406-3.047-1.078-4.344-2-0.875 0.156-1.797 0.25-2.75 0.25-2.828 0-5.422-0.781-7.375-2.063 0.453 0.031 0.922 0.063 1.375 0.063 3.359 0 6.531-0.969 8.953-2.719 2.609-1.906 4.047-4.484 4.047-7.281 0-0.812-0.125-1.609-0.359-2.375 2.641 1.453 4.359 3.766 4.359 6.375z",				
				Stats: 		"M0 26h32v4h-32zM4 18h4v6h-4zM10 10h4v14h-4zM16 16h4v8h-4zM22 4h4v20h-4z",
	...
	methods: {
		...
		loadTokenStats()
		{
			...
			.then(function (response)
			{
		        if (response.data)
		        {
		        	...
		        	self.aiinfo 		= response.data.aiinfo;
		...
		startKixote()
		{
			...
			if (urlParams.get('kixote') === 'help') {
				this.currentTab = 'Help';
				urlParams.delete('kixote');
				urlParams.delete('q');
...
kixote.component('tab-admin', {
	props: ['content', 'navigation', 'item', 'useragreement', 'aiservice', 'aiinfo', 'labels', 'settings', 'settingsSaved', 'kixoteSettings', 'urlinfo'],
	data: function () {
		return {
			messenger: [],
	...
	template: `<section class="dark:bg-stone-700 smooth-scroll dark:text-stone-200 bg-stone-200">
					<div class="p-5">
						<h1 class="mb-d3">Hello, I am <span class="text-teal-600">Kixote</span> from Typemill. Waiting for your commands.</h1>
					</div>
					<div>
...
kixote.component('tab-generate', {
	props: ['content', 'navigation', 'item', 'labels', 'urlinfo', 'settings', 'kixoteSettings', 'settingsSaved', 'aiservice', 'useragreement', 'aiinfo'],
	data: function () {
	    return {
	        tabs: [
	...
	template: `<section class="dark:text-stone-200">
				...
				<div v-if="!aiservice" class="dark:bg-stone-700 bg-stone-200 w-full p-5 dark:text-white">
				    <div class="p-5">
				        ...
				        <ol class="list-decimal list-inside space-y-2">
				            <li>Select an AI adapter.</li>
				            <li>Enter the API base URL.</li>
				            <li>Enter the model name.</li>
				            <li>Optionally enter an API key (not required for local providers).</li>
				        </ol>
				...
				<div v-else-if="!useragreement"  class="dark:bg-stone-700 bg-stone-200 w-full p-5 dark:text-white">
					<div class="p-5">
						<div class="w-full">
							<h2 class="text-xl font-bold mb-2">Agree to AI Usage</h2>
							<label class="flex items-start mb-2 mt-2">
								<input 
									...
									<span class="ml-2 text-sm">
									    By enabling AI features, you agree to the terms and conditions of
									    {{ aiinfo.providername || aiservice }}.
									    Your prompts and article content will be sent to that provider.
									    You can disable AI at any time in your user account.
									</span>
							</label>
							<div v-if="aiinfo.providerterms" class="text-sm mt-4">
								<p class="font-bold mt-2 mb-2">Terms:</p>
								<a :href="aiinfo.providerterms" class="text-teal-600" target="_blank">
									{{ aiinfo.providername || aiservice }} Terms &amp; Conditions
								</a>
							</div>
						</div>
					</div>
...
kixote.component('tab-help', {
	props: ['content', 'navigation', 'item', 'labels', 'urlinfo', 'settings', 'settingsSaved', 'aiservice', 'useragreement', 'aiinfo'],
	data: function () {
		return {
			messenger: [],
			command: '',
					...
					<div v-else>
						...
						<div class="p-5" v-if="!loading">
							<div class="w-full bg-stone-100 dark:bg-stone-600 p-2 flex justify-between">
								<p class="flex w-full">
									<span class="text-teal-600 p-1">Ki></span>
									<input type="text" ref="kinput" @keyup.enter="submitCommand" v-model.trim="command" class="flex-grow mr-1 bg-stone-100 dark:bg-stone-600 focus:outline-none border-0 dark:caret-white" placeholder="Ask a question about Typemill..." />
								</p>
						        <button
						        	class 	= "text-white px-2 py-1 bg-teal-600 hover:bg-teal-700"
						        	@click 	= "submitCommand"
	...
	methods: {
		...
		submitCommand()
		{
			if(this.command.trim() == '' || this.loading)
			{
				return;
			}

			var currentCommand = '<span class="text-teal-600 p-1">Ki></span> ' + this.esc(this.command);
			var message = { 'command' : currentCommand, 'answer' : ['<p class="text-stone-500 italic flex items-center gap-2"><span class="help-spinner"><span></span><span></span><span></span></span> Thinking ...</p>'] };
			this.messenger.push(message);

			var question = this.command.trim();
			this.command = '';
			this.loading = true;

			// Build history payload (last 6 completed turns from messenger)
			// Exclude the current message because its question is sent separately
			var history = [];
			var len = this.messenger.length;
			var start = Math.max(0, len - 7);
			for (var i = start; i < len - 1; i++) {
				var entry = this.messenger[i];
				// Extract plain text from command (strip the Ki> span)
				var userText = entry.command.replace(/<[^>]+>/g, '').replace(/^Ki>\s*/, '');
				if (userText) {
					history.push({ role: 'user', content: userText });
				}
				// The answer array contains HTML blocks; we need plain text for history
				if (entry.answer && entry.answer.length > 0) {
					var lastAnswer = entry.answer[entry.answer.length - 1];
					// Strip HTML tags to get plain text for history
					var plainText = lastAnswer.replace(/<[^>]+>/g, '');
					if (plainText !== '') {
			...
			{
				self.loading = false;
				var lastKey = self.messenger.length - 1;
				// Clear the "Thinking..." placeholder
				self.messenger[lastKey].answer = [];
				if (response.data.error) {
					self.messenger[lastKey].answer.push('<p class="text-rose-500">Error: ' + self.esc(response.data.error) + '</p>');
				} else {
					var answerHtml = self.renderMarkdown(response.data.answer || '');
					self.messenger[lastKey].answer.push(answerHtml);

					var sources = response.data.sources || [];
					if (sources.length > 0) {
						var sourceHtml = '<div class="mt-3 text-xs"><span class="font-bold">Read more:</span>';
						sources.forEach(function(src) {
							sourceHtml += ' <a href="https://docs.typemill.net' + self.esc(src.url) + '" target="_blank" class="text-teal-600 hover:underline">' + self.esc(src.title) + '</a>';
						});
						sourceHtml += '</div>';
						self.messenger[lastKey].answer.push(sourceHtml);
					}
				}
				self.finishCommand();
			})
			.catch(function (error)
			{
				...
				self.messenger[lastKey].answer.push('<p class="text-rose-500">Error: ' + self.esc(msg) + '</p>');
				self.finishCommand();
			});
		},
		renderMarkdown(text)
		{
			...
			function parseTable(startIndex) {
				...
				while (j < lines.length && lines[j].trim().startsWith('|')) {
					var cells = lines[j].split('|').map(function(c) { return c.trim(); }).filter(function(c) { return c !== ''; });
					rows.push(cells);
					j++;
				}
				...
				var header = rows[0];
				var bodyStart = isSep ? 2 : 1;
				var body = rows.slice(bodyStart);
				var out = '<table class="help-table"><thead><tr>';
				header.forEach(function(cell) {
					out += '<th>' + inlineMd(esc(cell)) + '</th>';
				});
				out += '</tr></thead><tbody>';
				body.forEach(function(row) {
					out += '<tr>';
					row.forEach(function(cell) {
						out += '<td>' + inlineMd(esc(cell)) + '</td>';
					});
					out += '</tr>';
				});
				out += '</tbody></table>';
				return { html: out, nextIndex: j };
			}
...
kixote.component('tab-info', {
	...
	template: `<section class="dark:bg-stone-700 dark:text-stone-200 bg-stone-200">
				<div class="p-5">
					<div class="dark:bg-stone-700 dark:text-stone-200 bg-stone-200 w-full p-5 dark:text-white">
						...
						<div v-if="aiservice">
							<p class="py-2">Active adapter: <strong>{{ aiinfo.adapter }}</strong></p>
							<p v-if="aiinfo.providername" class="py-2">Provider: <strong>{{ aiinfo.providername }}</strong></p>
							<p class="py-2">Default model: <strong>{{ aiinfo.model }}</strong></p>
							<p class="py-2">
								Selected model:
								<strong :class="selectedModel && selectedModel !== aiinfo.model ? 'text-teal-600' : ''">{{ selectedModel || aiinfo.model }}</strong>
								<span v-if="selectedModel && selectedModel !== aiinfo.model" class="ml-2 text-xs text-teal-600">(individual override)</span>
							</p>
									...
									<ul class="space-y-1">
										<li v-for="model in models" :key="model.id">
											<button
												...
												:class="[
													'w-full text-left px-3 py-2 border transition duration-100 text-sm font-mono',
													selectedModel === model.id
														? 'border-teal-600 bg-teal-600 text-white'
														: 'border-stone-300 dark:border-stone-500 hover:border-teal-600 hover:text-teal-600 dark:hover:border-teal-500'
												]"
											>{{ model.id }}</button>
										</li>
									</ul>
									<button
										v-if="selectedModel && selectedModel !== aiinfo.model"
										@click.prevent="clearModelOverride"
										class="mt-3 px-3 py-1 border border-stone-700 dark:border-stone-200 hover:bg-teal-600 hover:text-white hover:border-teal-600 transition duration-100 text-sm"
									>Reset to default model</button>
								</div>
								<div v-else>
									<p class="text-sm text-stone-500">No model list available from this provider.</p>
								</div>
								<button
									v-if="!modelsLoading"
									@click.prevent="loadModels"
									class="mt-3 px-3 py-1 border border-stone-700 dark:border-stone-200 hover:bg-teal-600 hover:text-white hover:border-teal-600 transition duration-100 text-sm"
								>{{ models.length > 0 ? 'Refresh model-list' : 'Load models' }}</button>
							</div>
	...
	mounted: function()
	{
		// Restore any previously selected model from localStorage
		var stored = localStorage.getItem('kixote_model_override');
		if (stored) {
			this.selectedModel = stored;
		} else {
			this.selectedModel = this.aiinfo.model || '';
		}
	},
	methods: {
		loadModels()
		{
			...
			tmaxios.get('/api/v1/kixote/models')
			.then(function (response)
			{
				...
				if (response.data && Array.isArray(response.data.models)) {
					...
					self.modelsError = 'No model list returned by provider.';
				}
			})
			.catch(function (error)
			{
				...
				if (error.response && error.response.data && error.response.data.message) {
					self.modelsError = error.response.data.message;
				} else {
					self.modelsError = 'Could not fetch model list from provider.';
				}
			});
		},
		selectModel(modelId)
		{
			...
			localStorage.setItem('kixote_model_override', modelId);
		},
		clearModelOverride()
		{
			this.selectedModel = this.aiinfo.model || '';
			localStorage.removeItem('kixote_model_override');
		},
	}
})